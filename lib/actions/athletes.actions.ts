...
export const getAthletes = cache(async (): Promise<{ data: OptimizedAthlete[], error: string | null }> => {
    try {
...
// Keep original function signature for getAthletesAction 
export const getAthletesAction = async (academicId: number): Promise<{ data: OptimizedAthlete[], error: string | null }> => {
    try {
        // Step 1: Get athletes data efficiently
        const athletesData = await db
            .select({
                id: academicAthletic.id,
                userId: academicAthletic.userId,
        ...
        // Step 2: Get bookings data separately
        const profileIds = athletesData
            .map(a => a.profileId)
            .filter((id): id is number => id !== null)
        ...
        // Step 3: Group bookings by profile ID
        const bookingsByProfile = bookingsData.reduce((acc, booking) => {
            if (!acc[booking.profileId]) {
                acc[booking.profileId] = []
            }
        ...
        console.error('Error fetching athletes for academy:', error)
        return {
            data: [],
            error: error instanceof Error ? error.message : 'Unknown error occurred'
        }
