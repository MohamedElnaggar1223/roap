...
import { invalidateGendersCache } from '@/lib/cache/invalidation'
...
export async function getPaginatedGenders(
    ...
    return await withPaginatedCache(
        CACHE_KEYS.PAGINATED_GENDERS(page, pageSize, orderByStr),
        async () => {
...
export const editGender = async (values: { name: string, id: number }) => {
    try {
        ...
        // Invalidate gender caches
        await invalidateGendersCache()
